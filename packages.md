...
| bluetui | bluetooth | Bluetooth TUI (scan, pair, connect), opened by the control panel's Bluetooth tile instead of raw `bluetoothctl`. Official `extra` repo. | pending |
...
| qimgv | viewer | Image viewer, the default for image files (set by `scripts/set-default-apps.sh` via `xdg-mime`, run from `install.sh`). **AUR** (not in the repos), builds from source via `yay`. | pending |
